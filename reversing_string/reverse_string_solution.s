...
find_end:
 lb t0, 0(a2)
 beq t0, zero, found ; check for end (null byte)
 addi a2, a2, 1
 j find_end

found:
addi a2, a2, -1 ; so we leave the null byte at the end and reverse only the string

; replace from both sides and meet in the middle
swap:
bge a1, a2, end_swap ; check if the pointers met
lb t0, 0(a2)
lb t1, 0(a1)
sb t0, 0(a1)
sb t1, 0(a2)
